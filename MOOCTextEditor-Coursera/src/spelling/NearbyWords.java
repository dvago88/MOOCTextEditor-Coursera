...
public class NearbyWords implements SpellingSuggest {
    ...
    public List<String> suggestions(String word, int numSuggestions) {
        ...
        while (!queue.isEmpty() && contador < numSuggestions) {
            List<String> mientras;
            String s = queue.pollFirst();
            mientras = distanceOne(s, true);


            while (!mientras.isEmpty() && contador < numSuggestions) {
                contador++;
                String palabra = mientras.remove(0);
                boolean comprobador = visited.add(palabra);
                if (comprobador) {
                    queue.add(palabra);
                    if (dict.isWord(palabra)) {
                        retList.add(palabra);
                    }
                }
            }
